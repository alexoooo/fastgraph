...
public class ItemWeights<I>
{
    ...
    public Collection<I> heaviest(int howMany, ItemFilter<I> filter)
    {
        ...
        SortedSet<WeightedItemLabel> weightedItemLabels =
                new TreeSet<WeightedItemLabel>();

        int curInitIndex = 0;
        for (int added = 0;
                curInitIndex < WEIGHTS.length && added < howMany;
                curInitIndex++)
        {
            I item = FOR_PLY.itemLabled(curInitIndex);
            if (! filter.accept( item )) continue;

            weightedItemLabels.add(
                    new WeightedItemLabel(
                            curInitIndex, WEIGHTS[curInitIndex]) );
            added++;
        }
        ...
        for (int i = curInitIndex; i < WEIGHTS.length; i++)
        {
            ...
            {
                WeightedItemLabel lightestHeavyWeight =
                        weightedItemLabels.first();
        ...
        {
            heaviest.addFirst(
                    FOR_PLY.itemLabled(weightedItemLabel.label) );
        }
    ...
    public static class Translator<I>
    {
        ...
        public Translator(
                int newFromOld[],
                FinalPly.Lookup<I> oldPly, FinalPly.Lookup<I> newPly)
        {
            OLD_PLY      = oldPly;
            NEW_PLY      = newPly;
        ...
        public ItemWeights<I> translate(
                ItemWeights<I> from, float commonFactor)
        {
                ...
                float commonFactor)
        {
            for (int label = 0; label < from.WEIGHTS.length; label++)
            {
                ...
                if (toCluster != null)
                {
                    to.WEIGHTS[ toCluster.label() ] =
                            commonFactor * from.WEIGHTS[ label ];
                }
            }
        }

        private void fastTranslate(
                ItemWeights<I> from,
                ItemWeights<I> to,
                float commonFactor)
        {
    ...
    private static class WeightedItemLabel
            implements Comparable<WeightedItemLabel>
    {
        public int   label;
        public float weight;
